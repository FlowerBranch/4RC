...
class SparseCorrespondences:
    ...
    def select_rows(self, mask: torch.Tensor) -> "SparseCorrespondences":
        """The rows a boolean mask keeps, still scene-level.

        Unlike :meth:`select_query_slot` this deliberately does **not** rebase
        ``query_slots``: the result still indexes the scene's anchor list, so it
        can be handed to :func:`gather_query_anchor_points` and sliced further
        with :meth:`anchor_rows`.  That is what lets a held-out subset be scored
        through exactly the same path as the supervised one, against the same
        alignment and the same gathered anchors.
        """

        mask = torch.as_tensor(mask)
        if mask.ndim != 1:
            raise ValueError(f"Row mask must be one-dimensional, got {mask.shape}")
        if mask.dtype is not torch.bool:
            raise ValueError(f"Row mask must be boolean, got {mask.dtype}")
        if mask.numel() != self.count:
            raise ValueError(
                f"Row mask covers {mask.numel()} rows but there are {self.count}"
            )
        mask = mask.to(self.trajectory_indices.device)
        return SparseCorrespondences(
            trajectory_indices=self.trajectory_indices[mask],
            query_slots=self.query_slots[mask],
            query_times=self.query_times[mask],
            rows=self.rows[mask],
...
@dataclass(frozen=True)
class HeldOutSplit:
    """A supervised/held-out partition of one scene's correspondences.

    ``supervised_rows`` and ``held_out_rows`` are boolean masks into the
    *scene-level* correspondence set they were built from, so anything gathered
    per-row from that set -- query pointmap anchors above all -- can be sliced
    the same way and stay aligned with both sides.
    """

    supervised: SparseCorrespondences
    held_out: SparseCorrespondences
    supervised_rows: torch.Tensor
    held_out_rows: torch.Tensor
    report: dict


HELD_OUT_SPLIT_RULE = (
    "Trajectories are banded along the world axis of largest spread, using each "
    "trajectory's position at its own query time. A band is held out when "
    "(band + offset) % period == 0; a trajectory within gutter_m of a held-out "
    "band edge is supervised by neither side."
)

# Ceiling on the share of correspondences the gutter may consume. ``gutter_m``
# is a fixed metric width while a band is ``extent / band_count`` wide, so on a
# scene with a short split axis the gutter eats most of every supervised band
# adjacent to a held-out one: at 2 m of extent and 16 bands the bands are
# 0.125 m and a 0.10 m gutter takes 80% of each edge-adjacent band. That shrinks
# *and biases* what the model trains on, which is the failure the query-time
# binding was introduced to avoid, so it must be loud rather than silent.
# Deliberately not a flag, for the same reason ``min_held_out_rows`` is not one:
# a knob that raises the ceiling is a knob for making this quiet.
MAX_GUTTER_SHARE = 1 / 3


def _held_out_split_report(
    *,
    enabled,
    supervised_row_count,
    held_out_row_count,
    gutter_row_count,
    realized_held_out_fraction,
    axis=None,
    axis_extent_m=None,
    band_count=None,
    band_width_m=None,
    ...
    band_offset=None,
    gutter_m=None,
    held_out_band_indices=None,
    supervised_trajectory_count=None,
    held_out_trajectory_count=None,
    min_query_time_separation_m=None,
    min_window_separation_m=None,
    per_anchor_held_out_counts=None,
) -> dict:
    """One key set for both branches, with None where nothing was computed.

    This report goes verbatim into ``run_summary.json``, so a collection script
    that reads a geometry field must find that key on *every* archived run --
    including the ones with the split off, which is the default. Two branches
    emitting two shapes turns "this run had no split" into a KeyError in the
    aggregator, which is a much worse way to learn it.
    """

    return {
        "rule": HELD_OUT_SPLIT_RULE,
        "enabled": enabled,
        "axis": axis,
        "axis_extent_m": axis_extent_m,
        "band_count": band_count,
        "band_width_m": band_width_m,
        "period": period,
        "band_offset": band_offset,
        "gutter_m": gutter_m,
        "held_out_band_indices": held_out_band_indices,
        "supervised_row_count": supervised_row_count,
        "held_out_row_count": held_out_row_count,
        "gutter_row_count": gutter_row_count,
        "supervised_trajectory_count": supervised_trajectory_count,
        "held_out_trajectory_count": held_out_trajectory_count,
        "realized_held_out_fraction": realized_held_out_fraction,
        "min_query_time_separation_m": min_query_time_separation_m,
        "min_window_separation_m": min_window_separation_m,
        "per_anchor_held_out_counts": per_anchor_held_out_counts,
    }


def _held_out_split_axis(positions: np.ndarray) -> int:
    """The world axis the query points spread along the furthest.

    Splitting along the axis the scene actually extends in is what makes the
    gutter buy real separation: on a flat axis every band edge would be inside
    the point cloud's noise.
    """
...
def split_correspondences_by_region(
    ...
    *,
    ...
    band_offset: int = 0,
    gutter_m: float = 0.10,
    min_held_out_rows: int = 32,
) -> HeldOutSplit:
    """Partition correspondences into a supervised and a scored-only set.

    **What a held-out score does and does not catch.** These rows catch the
    encoder fitting the *supervised* rows specifically -- a real risk at a high
    ``encoder_lr`` with hundreds of millions of trainable parameters on a single
    scene. They do **not** catch a mostly-global transform of the whole track
    field, which hurts both sides equally; the alignment-scale drift figures are
    what catch that. Nor do they bound *same-rigid-body* leakage: Kubric scenes
    are rigid movers, so two trajectories on one object have displacement fields
    related by a single rigid transform, and no amount of spatial separation
    breaks that. The gutter bounds spatial leakage only. A per-object holdout
    would close the remaining route, but the dump carries no segmentation and no
    instance ids, so bands are the best available split without re-dumping.
    All three measurements are needed; none substitutes for another.

    **Why query time.** Each trajectory is placed by where it is at its *own*
    query frame, one position per trajectory. Binding membership to every
    selected time instead would drop any trajectory that crosses a band boundary
    during the window into the gutter -- which would strip fast movers out of the
    *supervised* set, exactly the trajectories where the track field is hard.
    That would bias what the model trains on, not just what it is scored on.
    Query time is also the right time on its own terms: memorization is indexed
    by the query anchor, so two rows separated as queries are the ones that are
    not interpolations of each other.

    **The separation guarantee.** Since ``|dp_axis| <= ||dp||``, an axis-aligned
    gutter of ``g`` metres implies at least ``g`` metres of 3D separation
    between any supervised and any held-out query point. ``report`` records the
    realised minimum rather than restating the claim.

    ``holdout_fraction=0`` disables the split: everything is supervised, the
    held-out set is empty, and the minimum-rows check does not apply.
    """

    if not np.isfinite(holdout_fraction) or not 0 <= holdout_fraction <= 0.5:
        raise ValueError("holdout_fraction must be finite and in [0, 0.5]")
    if band_count < 2:
        raise ValueError("band_count must be at least 2")
    if band_offset < 0:
        raise ValueError("band_offset must be non-negative")
    if not np.isfinite(gutter_m) or gutter_m < 0:
        raise ValueError("gutter_m must be finite and non-negative")
    if correspondences.count == 0:
        raise ValueError("No eligible sparse correspondences to split")

    row_count = correspondences.count
    if holdout_fraction == 0:
        supervised_rows = torch.ones(row_count, dtype=torch.bool)
        held_out_rows = torch.zeros(row_count, dtype=torch.bool)
        return HeldOutSplit(
            supervised=correspondences,
            held_out=correspondences.select_rows(held_out_rows),
            supervised_rows=supervised_rows,
            held_out_rows=held_out_rows,
            report=_held_out_split_report(
                enabled=False,
                supervised_row_count=row_count,
                held_out_row_count=0,
                gutter_row_count=0,
                realized_held_out_fraction=0.0,
            ),
        )

    period = max(2, int(round(1.0 / holdout_fraction)))
    if band_count < period:
        raise ValueError(
            f"band_count={band_count} is below the period {period} implied by "
            f"holdout_fraction={holdout_fraction}, so no band would be held out"
        )

    trajectory_indices = correspondences.trajectory_indices.cpu().numpy()
    query_points = scene.query_points.detach().cpu().numpy().astype(np.float64)
    # query_points[:, 1:] is the trajectory's position at its own query time;
    # build_anchor_correspondences already cross-checks it against
    # traj3d_world at that frame, so this needs no second gather.
    positions = query_points[trajectory_indices, 1:]

    axis = _held_out_split_axis(positions)
    coordinate = positions[:, axis]
    low = float(coordinate.min())
    high = float(coordinate.max())
    extent = high - low
    if not np.isfinite(extent) or extent <= 0:
        raise RuntimeError(
            "Query points have no extent along any world axis, so no spatial "
            "split exists; run with --holdout_fraction 0"
        )
    band_width = extent / band_count
    band = np.clip(
        np.floor((coordinate - low) / band_width).astype(np.int64),
        0,
        band_count - 1,
    )
    held_out_band = ((band + band_offset) % period) == 0

    # Distance to the nearest edge of a held-out band, measured along the split
    # axis. A supervised row inside the gutter is dropped from both sides.
    held_out_band_indices = sorted(
        {
            index
            for index in range(band_count)
            if ((index + band_offset) % period) == 0
        }
    )
    edges = np.array(
        [low + index * band_width for index in held_out_band_indices]
        + [low + (index + 1) * band_width for index in held_out_band_indices],
        dtype=np.float64,
    )
    distance_to_edge = np.abs(coordinate[:, None] - edges[None, :]).min(axis=1)

    held_out_mask = held_out_band
    supervised_mask = ~held_out_band & (distance_to_edge >= gutter_m)
    gutter_mask = ~held_out_mask & ~supervised_mask

    # One row per trajectory is what build_anchor_correspondences emits, and the
    # band test is per trajectory, so a trajectory cannot straddle the split.
    # Assert it rather than trust it: this is what the whole guarantee rests on.
    for name, mask in (
        ("supervised", supervised_mask),
        ("held_out", held_out_mask),
    ):
        other = trajectory_indices[~mask]
        if np.intersect1d(trajectory_indices[mask], other).size:
            raise RuntimeError(
                f"A trajectory appears both inside and outside the {name} set; "
                "the split would leak across itself"
            )

    supervised_rows = torch.from_numpy(supervised_mask)
    held_out_rows = torch.from_numpy(held_out_mask)
    supervised_count = int(supervised_mask.sum())
    held_out_count = int(held_out_mask.sum())
    gutter_count = int(gutter_mask.sum())

    if supervised_count == 0:
        raise RuntimeError(
            "The region split left no supervised correspondences "
            f"({held_out_count} held out, {gutter_count} in the gutter of "
            f"{row_count} rows). Lower --holdout_fraction or --holdout_gutter_m."
        )
    # Checked before the held-out floor below, because a gutter this wide
    # explains a shrunken supervised set directly, while the floor would only
    # report a symptom.
    #
    # Note the two messages advise --holdout_band_count in *opposite*
    # directions, and both are right: this one wants fewer, wider bands so a
    # fixed-width gutter is a smaller share of each; the floor below wants more
    # bands so the held-out sample is spread over more regions. They pull
    # against each other because band width is what they both move, and the
    # workable range is between them -- if no band_count satisfies both, the
    # scene's split axis is too short for a spatial holdout at this gutter.
    if gutter_count > row_count * MAX_GUTTER_SHARE:
        raise RuntimeError(
            f"The region split put {gutter_count} of {row_count} rows in the "
            f"gutter, above the {MAX_GUTTER_SHARE:.0%} ceiling "
            f"({supervised_count} supervised, {held_out_count} held out). "
            f"A {gutter_m:.3f} m gutter is most of a {band_width:.3f} m band, "
            "so it is eating the supervised set rather than separating it: "
            "lower --holdout_band_count for wider bands, or lower "
            "--holdout_gutter_m."
        )
    if held_out_count < min_held_out_rows:
        raise RuntimeError(
            f"The region split held out only {held_out_count} rows, below the "
            f"{min_held_out_rows} needed for the score to mean anything "
            f"({supervised_count} supervised, {gutter_count} in the gutter of "
            f"{row_count} rows; realised fraction "
            f"{held_out_count / row_count:.3f}). Raise --holdout_fraction, "
            "raise --holdout_band_count, or lower --holdout_gutter_m."
        )

    supervised_positions = positions[supervised_mask]
    held_out_positions = positions[held_out_mask]
    min_query_separation = float(
        np.linalg.norm(
            supervised_positions[:, None, :] - held_out_positions[None, :, :],
            axis=2,
        ).min()
    )
    # Diagnostic only, and deliberately not enforced: trajectories that are
    # separated as queries are free to approach each other later in the window.
    # Reporting it keeps that visible instead of letting the query-time figure
    # be read as a guarantee about the whole window.
    trajectories = scene.trajectories_world.detach().cpu().numpy().astype(np.float64)
    supervised_trajectories = np.unique(trajectory_indices[supervised_mask])
    held_out_trajectories = np.unique(trajectory_indices[held_out_mask])
    min_window_separation = min(
        float(
            np.linalg.norm(
                trajectories[time, supervised_trajectories][:, None, :]
                - trajectories[time, held_out_trajectories][None, :, :],
                axis=2,
            ).min()
        )
    ...
    report = _held_out_split_report(
        ...
        held_out_row_count=held_out_count,
        gutter_row_count=gutter_count,
        supervised_trajectory_count=int(
            np.unique(trajectory_indices[supervised_mask]).size
        ),
        held_out_trajectory_count=int(
            np.unique(trajectory_indices[held_out_mask]).size
        ),
        realized_held_out_fraction=held_out_count / row_count,
        # The enforced figure: at least gutter_m by construction, since the
        # axis distance lower-bounds the 3D distance.
        min_query_time_separation_m=min_query_separation,
        # Not enforced -- see the comment where it is computed.
        min_window_separation_m=min_window_separation,
        per_anchor_held_out_counts={
            int(slot): int(
                (correspondences.query_slots.cpu().numpy()[held_out_mask] == slot).sum()
            )
            for slot in sorted(
                set(correspondences.query_slots.cpu().numpy().tolist())
            )
        },
    )
    return HeldOutSplit(
        supervised=correspondences.select_rows(supervised_rows),
        held_out=correspondences.select_rows(held_out_rows),
        supervised_rows=supervised_rows,
        held_out_rows=held_out_rows,
        report=report,
