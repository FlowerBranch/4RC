...
class _FakeMotionDecoder(nn.Module):
    def __init__(self):
        ...
        # Recorded so the merge tests can assert what Arc derived and which
        # branch it asked for; the real decoder emits S // views_per_time rows
        # under merge and S rows otherwise, mirrored below, and the defaults
        # keep every pre-merge test's behaviour unchanged.
    ...
    def forward(
        ...
        self.seen_merge.append(merge)
        B, S, _, C = tokens.shape
        rows = S // views_per_time if merge else S
        return torch.full(
            (B, rows, 2, C),
